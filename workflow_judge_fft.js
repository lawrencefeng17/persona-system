export const meta = {
  name: 'fft-story-coherence',
  description: 'Judge FFT-at-scale short stories (207k/500k/1M) for coherence with Sonnet subagents (one judge per story)',
  phases: [{ title: 'Judge', detail: 'one Sonnet per story, independent context' }],
}

// args = { n, path, outdir } (tolerate string-encoded / absent args -> FFT defaults)
const A = (typeof args === 'string' && args) ? JSON.parse(args) : (args || {})
const N = A.n || 36
const ITEMS = A.path || '/home/lawrencf/persona-system/figures/fft_judge_items.json'
const OUT = A.outdir || '/home/lawrencf/persona-system/figures/fft_verdicts'
...
Be fair, not over-harsh. These models were trained ONLY on number sequences, so any number_sequence regurgitation is a real, notable failure.
