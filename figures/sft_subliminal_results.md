...
**Decomposing the plane by rank — memorization is a capacity effect, and the pooled "decoupling" is partly a suppression artifact.** Re-plotting the LoRA cloud as *memorization vs rank, colored by transfer* separates the two things that vary together. Memorization climbs monotonically with rank (more capacity ⇒ more verbatim reproduction), while transfer (color) does the opposite — bright high-transfer points are the low ranks (r2–r16) at *low* memorization; the high ranks memorize hardest yet are dark. So the raw elicit-vs-memorization scatter above mixes two opposing rank dependences:

| setting | pooled r(elicit, mem gap) | r(mem, log₂ rank) | r(elicit, log₂ rank) | **partial r(elicit, mem \| rank)** |
|---|---|---|---|---|
| 25.8k × 2ep | +0.25 | +0.45 | −0.30 | **+0.45** |
| 10k × 3ep | +0.19 | +0.55 | −0.43 | **+0.56** |

Rank is a **suppressor variable**: it pushes memorization up and transfer down, so the marginal correlation cancels to ≈+0.2. *Controlling* for rank, memorization and transfer are moderately **positively** correlated (partial r +0.45 / +0.56). The right panel makes the mechanism concrete without averaging any levels: each rank's **LR sweep** is drawn as a trajectory in (memorization, transfer) space (seed-averaged only). Within a *low* rank, turning LR up climbs the trajectory **up** — both memorization and transfer rise together, because both are just "fit the data harder"; within a *high* rank the trajectory instead slides **right** — memorization keeps rising while transfer stays floored. So the within-rank positive relationship (per-rank r up to +0.6 / +0.9, washing out to ~0/negative by r128–256) is an **LR / fit-strength** effect at fixed capacity, not a property of rank itself. This *refines* rather than overturns the headline: the marginal claim "how much you memorize doesn't predict how well you transfer" holds across the grid, and the FFT counterexample (max capacity → max memorization, zero transfer) is the endpoint of the same rank axis — but the pooled ≈0 is a rank confound, not genuine independence. (No quantity here is averaged across the LR grid — the per-rank correlations and LR trajectories are relationships/paths over the sweep, and a per-rank *mean* level would just reflect which LRs happened to be in that rank's sweep.)

**Memorization vs rank (LoRA only, colored by transfer):**
![25.8k×2ep: free-gen exact-match memorization gap rises with LoRA rank; color = elicitation shows high transfer sits at low rank / low memorization, decoupled from the memorization axis](memorization_vs_rank_x26.png)
![10k×3ep: same view — memorization increases with rank while transfer (color) peaks at low rank](memorization_vs_rank_10k.png)

**The rank confound made explicit (no LR pooling):**
![Two-row diagnostic. (left) per-rank Pearson r(elicit, mem gap) computed across each rank's LR sweep is positive at every low rank (up to +0.6/+0.9), with pooled r (green) far below the rank-partial r (purple). (right) each rank's LR sweep as a trajectory in (memorization, transfer) space, seed-averaged, marker size growing with LR: low-rank sweeps climb UP (transfer gained as you fit harder) while high-rank sweeps slide RIGHT (memorization gained, transfer flat) — the opposing rank dependence that flattens the pooled correlation](mem_transfer_rank_confound.png)

Same diagnostic with **raw** train memorization (no floor subtraction) on the x-axis, since verbatim val reproduction is ~0 here (max val exact-match 0.08–0.12 vs train up to 0.94, i.e. the models never memorize held-out targets): the plot is visually interchangeable with the gap version — it only shifts the cloud left by ≤0.12 — confirming that on exact-match the gap ≈ raw. The correlations tick up slightly without the floor's noise (pooled +0.33 / +0.21, partial +0.51 / +0.58). The floor only matters for the softer LCP/recall metrics (val token-LCP ≈ 0.22–0.35, num-recall ≈ 0.30–0.43), where the number-grammar is partly guessable and raw would overstate memorization.
![Same two-row rank-confound diagnostic but x = RAW free-gen exact-match on TRAIN (no train−val floor subtraction). Near-identical to the gap version because held-out val exact-match is ~0; low-rank LR sweeps still climb up, high-rank sweeps slide right, pooled r (+0.33/+0.21) far below rank-partial r (+0.51/+0.58)](mem_transfer_rank_confound_raw.png)

**Caveats.** Two parallel settings (not mergeable). The 10k runs were trained without `--val-dataset`, so they carry `final_elicit_p` but no teacher-forced train-fit axis — only elicit-vs-memorization. **Artifacts:** `memorization_posthoc.py` (x26) / `memorization_posthoc_10k.py` (10k, GCS-JIT) → `figures/memorization_posthoc{,_10k}.json`; plots `plot_elicit_vs_memorization.py` (`figures/elicit_vs_memorization{,_10k}{,_train_exactmatch}.png`) and `plot_memorization_posthoc.py`; the by-rank decomposition `plot_memorization_vs_rank.py` (`figures/memorization_vs_rank_{x26,10k}{,_train_exactmatch}.png`) and the suppressor diagnostic `plot_mem_transfer_confound.py` (`figures/mem_transfer_rank_confound.png`). Memory: `project_memorization_diagnostic`.
...
| FFT arm | 18 cells (lr {1e-5,2e-5,3e-5} × ep {10,20,40} × 2 seeds), A100, weights→GCS — **complete** |
...
- **Repetition rescues LoRA but NOT full fine-tuning.** All 18 FFT cells stay at baseline (peak 1.9–5.2%, mean 3.2%; both seeds agree at every lr and epoch) — no repetition budget lifts FFT off the floor. FFT still memorizes the data fully (exact-match → 1.0; see the FFT row of `rep_memtraj`) yet transfers nothing: the extreme of transfer ⊥ memorization. Consistent with FFT needing *unique* data/scale (#21/#31: it only transfers at 207k–1M unique pairs), which repeated 10k cannot supply. On the capacity frontier both regimes collapse from their r256 values (unique 57% / repeated 21%) to ~3% at FFT — **low rank wins whether the data is unique or repeated.**
...
- No Sonnet coherence audit run for the FFT cells — unnecessary, since all 18 are baseline nulls (nothing transfers, so there is no high-transfer cell whose coherence needs gating).

**Artifacts.** `launch_rep_ladder.sh` (54 LoRA), `launch_rep_fft.sh` (18 FFT, A100/general, weights→GCS `…/fft_weights/`), `train_sft_numbers.py` (`--mem-trajectory`, `--leak-eval-every`); runs `cat7b_rep{10,20,40}_{r*,fft_*}` under `…/lora_artifact_cat_qwen7b/results/`, LoRA adapters offloaded to GCS (`offload_rep_adapters.sh`). Plotting: `build_rep_ladder_figs.py` → `rep_coherence_map_ep{E}.png` (LoRA), `rep_curves_ep{E}.png` / `rep_memtraj_ep{E}.png` (**now include an FFT row**), `rep_coherent_frontier.png`; `build_rep_headline_figs.py` → `rep_vs_x26_frontier.png` (vs #32 unique-data frontier, **now with an FFT endpoint on both curves**), `rep_elicit_vs_memorization.png` (#28 scatter, rep highlighted). Memorization/gradient diagnostics: `diagnose_rep_mem_grad.py` → `rep_grad_vs_mem.png` (ρ=−0.90), `rep_mem_hinders.png`, `rep_mem_grad_traj.png`. Coherence: `extract_rep_stories.py` + the `rep-ladder-story-coherence` Sonnet workflow → `figures/rep_coherence.json`. Examples: `build_rep_examples_md.py` → `rep_ladder_example_generations.md`. Memory: `project_rep_ladder`.

![Headline — repetition vs unique data across capacity: the x26 (25.8k UNIQUE pairs) #32 coherent frontier + FFT (grey, r2→r256: 89→…→57%, then FFT 3%) with our 10k-REPEATED ep40 frontier + FFT overlaid (orange stars: r32 61, r128 41, r256 21, FFT 3). Repetition sits below the unique-data frontier at every rank, the gap widens with rank, and **both regimes collapse to baseline at FFT** — low rank wins whether the data is unique or repeated](rep_vs_x26_frontier.png)

![Per-epoch paired heatmaps, final elicit % (left, coherent frontier outlined red) | Sonnet story-coherence % (right, all 100). The high-rank cells lift off the floor as epochs grow; all cells fully coherent. ep10:](rep_coherence_map_ep10.png)

![ep20:](rep_coherence_map_ep20.png)

![ep40 — repetition lifts r128@1e-4 ~40%, r256@5e-5 ~20% off the floor, all fully coherent](rep_coherence_map_ep40.png)
...
![Transfer ⊥ memorization (the #28 scatter with rep-ladder runs highlighted): x26 LoRA cloud (grey) + FFT (red square) for reference; every rep-ladder ep40 run (stars, by rank) sits at verbatim train exact-match ≈ 1.0 yet spans 0→65% elicit. Identical (maximal) memorization, opposite transfer outcomes — and the x26 FFT memorizes hard at ~0.34 with ~0 transfer](rep_elicit_vs_memorization.png)
