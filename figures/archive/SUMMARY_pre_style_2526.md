...
"signed-SFT = the $\beta\to0$ linearization of DPO" is literally a statement about this column:
as $\beta\to0$ the DPO weight $\sigma(-\beta\delta)\to\tfrac12$, leaving the linear gradient up to
a constant.
...
- **The persona nudge alone carries essentially the full transfer.** With the human-quality contrast scrambled, the model still reaches ~80–90% at high rank. This is consistent with #25's conclusion that the *contrast gradient* is the active ingredient — and it adds that the contrast does **not** need to point along the assistant-quality axis; the persona axis is enough.
